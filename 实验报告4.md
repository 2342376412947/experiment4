...
#### 二、实验原理
1.多层感知机（MLP）是由输入层、隐藏层与输出层构成的全连接神经网络，其核心工作机制是通过 “线性变换 + 非线性激活” 的组合拟合复杂数据分布。其中，全连接层作为核心组件，会对输入特征执行矩阵运算 
y=X⋅W+b
（其中 
X
 代表输入特征矩阵，
W
 为权重矩阵，
b
 为偏置项），实现特征的维度转换与线性映射；为打破线性模型的表达局限，网络引入非线性激活函数（本实验选用 ReLU 函数），让模型能够学习数据中的非线性关系；损失函数方面，因任务为多分类问题，采用交叉熵损失来量化预测结果分布与真实标签之间的差异；参数优化则通过随机梯度下降（SGD）算法完成，不断迭代更新权重矩阵 
W
 和偏置 
b
，最终实现损失函数的最小化，让模型具备稳定的分类能力。

2.手动实现全连接层的核心是矩阵乘法与梯度传播，需显式完成三步骤：先以正态分布等方式初始化权重矩阵
W
和偏置
b
；前向传播用torch.matmul实现
y=X⋅W+b
，搭配 ReLU 激活；反向传播可手动计算损失对
W
、
b
的梯度，或借助自动求导获取梯度后手动更新参数。
...
API 实现：代码简洁，隐藏了参数定义、梯度计算等细节，适合快速开发。

手动实现：需显式处理矩阵运算与参数更新，更能体现全连接层的数学本质，适合理解原理。
